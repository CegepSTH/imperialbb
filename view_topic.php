...
if(isset($_POST['vote'])) {
	...
	$db->query("UPDATE `".$db_prefix."pollvotes` SET `poll_choice_votes` = (poll_choice_votes+1) WHERE `poll_choice_id` = '".$_POST['poll_vote_choice']."' AND `poll_topic_id` = '".$_GET['tid']."'");
...
$topic_sql = $db->query("SELECT count(p.`post_id`) AS 'post_count', t.*, f.`forum_id`, f.`forum_name`, f.`forum_cat_id`, f.`forum_type`, f.`forum_read`, f.`forum_reply`, f.`forum_mod`, g.`ug_read`, g.`ug_reply`, g.`ug_mod`
...
if($topic = $db->fetch_array($topic_sql))
{
	...
	$db->query("UPDATE `".$db_prefix."topics` SET `topic_views` = '$new_topic_views' WHERE `topic_id` = '"  . $topic['topic_id'] .  "'");
	...
	if($topic['forum_type'] == "f")
	{
		...
		{
			$forum_route_query = $db->query("SELECT `forum_id`, `forum_cat_id`, `forum_name`, `forum_type` FROM `".$db_prefix."forums` WHERE `forum_id` = '".$forum_route_next."'");
	...
	if(!empty($topic['topic_poll_title'])) {
		...
		if(isset($_GET['view_poll_results']) && $_GET['view_poll_results'] == 1 || $vote_cast) {
			$query = $db->query("SELECT SUM(t.`poll_choice_votes`) AS 'poll_total_votes', p.`poll_choice_name`, p.`poll_choice_votes`
								FROM (`".$db_prefix."pollvotes` p
								LEFT JOIN `".$db_prefix."pollvotes` t ON t.`poll_topic_id` = '".$_GET['tid']."')
								WHERE p.`poll_topic_id` = '".$_GET['tid']."'
			...
			$query = $db->query("SELECT `poll_choice_id`, `poll_choice_name`
								FROM `".$db_prefix."pollvotes`
								WHERE `poll_topic_id` = '".$_GET['tid']."'
								ORDER BY `poll_choice_id`");
			while($poll = $db->fetch_array($query)) {
	...
	$newtime = 0;
	$post_sql = $db->query("SELECT p.`post_id`, p.`post_timestamp`, p.`post_text`, u.`user_id`, u.`username`, u.`user_date_joined`, u.`user_posts`, u.`user_signature`, u.`user_avatar_type`, u.`user_avatar_location`, u.`user_website`, u.`user_location`, r.`rank_name`, r.`rank_image`
			       FROM ((`".$db_prefix."posts` p
			       LEFT JOIN `".$db_prefix."users` u ON p.`post_user_id` = u.`user_id`)
			       LEFT JOIN `".$db_prefix."ranks` r ON r.`rank_id` = u.`user_rank`)
			       WHERE `post_topic_id` = '"  . $_GET['tid'] .  "'
			       ORDER BY `post_id` LIMIT ".$pp->limit."");

	while ($post = $db->fetch_array($post_sql))
	{
		// Try give the user a rank if they dont already have one
		if(empty($post['rank_name']) && $post['user_id'] > 0)
		{
			$rank_query = $db->query("SELECT `rank_name`
								FROM `".$db_prefix."ranks`
								WHERE `rank_special` = 0 AND `rank_minimum_posts` < '".$post['user_posts']."'
								ORDER BY `rank_minimum_posts` DESC
								LIMIT 1");

			if($rank_result = $db->fetch_array($rank_query))
			{
